    ...
    data.courses.forEach(function (element) {
      ...
      client.getCourseModules(element.id, function (error, modules) {
        modules.forEach(function (module) {
          ...
          client.getModuleLectures(module.id, function (error, list) {

            list.forEach(function (list) {
              var li_list = document.createElement("li");
              li_list.innerText = list.title;
              ul_list.appendChild(li_list);
            });
            div.onclick = function () {
              ul_list.classList.toggle('visible');
              ul_list.classList.toggle('custom-jumbotron');
            };
          });
        });
      });
...
>>>>>>> 14af4677591f24a303ad5263b1448aecca288c79
